...
# banzeimykali.sh Modify: H1r0t0
# git clone https://github.com/Dewalt-arch/pimpmykali 
# Usage: sudo ./banzeimykali.sh  ( defaults to the menu system )
...
check_for_root() {
    if [ "$EUID" -ne 0 ]
      then echo -e "\n\n Script must be run with sudo ./banzeimykali.sh or as root \n"
      exit
    else
...
clean_kali_basic() {
    echo -e "\n  ${greenplus} Cleaning APT cache and removing orphaned packages"
    APP="apt"
    FUNCTYPE="clean"
    apt clean
    EXIT_STATUS="$?"
    ...
    APP="apt"
    FUNCTYPE="autoremove --purge"
    apt autoremove --purge -y
    EXIT_STATUS="$?"
    check_exit_status ${APP} ${FUNCTYPE} ${EXIT_STATUS}
    clean_vars

    echo -e "\n  ${greenplus} Vacuuming journalctl logs to 100M"
    journalctl --vacuum-size=100M
    echo -e "${spaces}${greenplus} Basic cleanup complete"
    }


clean_kali_zeroout_interrupt() {
    echo -e "\n\n  ${redexclaim} Zero-Out interrupted (Ctrl+C) - removing /zero.file"
    sync
    if [ -f /zero.file ]
      then
        rm -f /zero.file
        echo -e "${spaces}${greenplus} /zero.file removed"
      else
        echo -e "${spaces}${greenminus} /zero.file not found"
    fi
    echo -e "\n  ${redexclaim} Zero-Out was interrupted - reclaimed space may be partial"
    echo -e "${spaces}${greenminus} You can shutdown the VM and run Compact/Compress now"
    exit 1
    }


clean_kali_zeroout() {
    echo -e "\n  ${redexclaim} WARNING ${redexclaim} Zero-Out fills all free disk space with a temp file so the"
    echo -e "  hypervisor (VMware/VirtualBox) can reclaim space on Compact/Compress."
    echo -e "  This can take a long time, and 'No space left on device' near the end is expected."
    echo -e "  Press [Ctrl+C] at any time to stop and clean up the temp file safely.\n"
    read -n1 -p "  Press Y to start Zero-Out, any other key to skip: " zeroout_userinput
    case $zeroout_userinput in
      y|Y) ;;
        *) echo -e "\n\n  ${redexclaim} Skipping Zero-Out"; return;;
    esac

    trap clean_kali_zeroout_interrupt SIGINT SIGTERM
    sync
    dd if=/dev/zero of=/zero.file bs=4M status=progress || true
    trap - SIGINT SIGTERM

    echo -e "\n  ${greenplus} dd finished, removing /zero.file"
    sync
    rm -f /zero.file
    echo -e "${spaces}${greenplus} Zero-Out complete"
    echo -e "${spaces}${greenplus} Shutdown this VM, then use the hypervisor's Compact/Compress disk feature"
    }
...
fix_kali_lightdm_theme_and_background () {
    ...
    if [ ! -f ${LIGHTDM_GTK_GREETER_CONF} ]
      then
        echo -e "\n  ${greenminus} ${LIGHTDM_GTK_GREETER_CONF} not found (lightdm not in use, e.g. KDE/sddm) - skipping"
        return
    fi

    APP="sed"
    FUNCTYPE="update lightdm-gtk-greeter.conf"
    sed s:"Kali-Light":"Kali-Dark":g -i ${LIGHTDM_GTK_GREETER_CONF}
    EXIT_STATUS="$?"
    check_exit_status ${APP} ${FUNCTYPE} ${EXIT_STATUS}
    clean_vars
    }
...
fix_netexec() {
    ...
    if [[ $findrealuser == "root" ]];
     then
       ...
       NETEXEC_VENV_DIR="$HOME/.local/share/pipx/venvs/netexec"
       if [ -d "$NETEXEC_VENV_DIR" ] && [ ! -e "$NETEXEC_VENV_DIR/bin/python" ]
         then
           echo -e "${spaces}${redexclaim} Existing netexec venv has a broken interpreter (likely a stale venv from a system python upgrade), removing before reinstall"
           pipx uninstall netexec >/dev/null 2>&1
       fi
     ...
     if [[ $findrealuser != "root" ]];
      then
        ...
        sudo -i -u $findrealuser sh -c 'NETEXEC_VENV_DIR="$HOME/.local/share/pipx/venvs/netexec"; if [ -d "$NETEXEC_VENV_DIR" ] && [ ! -e "$NETEXEC_VENV_DIR/bin/python" ]; then pipx uninstall netexec >/dev/null 2>&1; fi; pipx install git+https://github.com/Pennyw0rth/NetExec --force'
        getshell=$(echo $SHELL | cut -d "/" -f4)
...
fix_certipy() {
    is_installed "certipy-ad"
    }


fix_coercer() {
    is_installed "coercer"
    }


fix_subfinder() {
    is_installed "subfinder"
    }


fix_httpx() {
    is_installed "httpx-toolkit"
    }
...
fix_katana() {
    echo -e "\n  ${greenplus} Installing katana (projectdiscovery)"
    is_installed "golang"
    fix_go_path

    GOBIN_PATH="/home/$finduser/go/bin"
    [ "$finduser" == "root" ] && GOBIN_PATH="/root/go/bin"

    APP="go"
    FUNCTYPE="install katana"
    sudo -i -u $finduser go install github.com/projectdiscovery/katana/cmd/katana@latest
    EXIT_STATUS="$?"
    check_exit_status ${APP} ${FUNCTYPE} ${EXIT_STATUS}
    clean_vars

    if [ -f ${GOBIN_PATH}/katana ]
      then
        ln -sf ${GOBIN_PATH}/katana /usr/local/bin/katana
        echo -e "${spaces}${greenplus} katana installed, symlinked to /usr/local/bin/katana"
      else
        echo -e "${spaces}${redexclaim} katana binary not found at ${GOBIN_PATH}/katana"
    fi
    }


fix_projectdiscovery() {
    echo -e "\n  ${greenplus} Installing ProjectDiscovery Suite (subfinder, httpx-toolkit, nuclei, dnsx, katana)"
    fix_subfinder
    fix_httpx
    fix_nuclei
    fix_dnsx
...
fix_autorecon() {
    echo -e "\n  ${greenplus} Installing AutoRecon (automated multi-threaded recon orchestration)"
    is_installed "autorecon seclists curl dnsrecon enum4linux feroxbuster gobuster impacket-scripts nbtscan nikto nmap onesixtyone oscanner redis-tools smbclient smbmap snmp sslscan sipvicious tnscmd10g whatweb"
    }


start_engagement_log() {
    is_installed "asciinema"

    LOG_DIR="/home/$finduser/engagement-logs"
    [ "$finduser" == "root" ] && LOG_DIR="/root/engagement-logs"
    [ ! -d ${LOG_DIR} ] && mkdir -p ${LOG_DIR}

    echo -e "\n  ${greenplus} Engagement Session Logger"
    read -p "  Enter a short tag for this session (e.g. client/engagement name): " engagement_tag
    [ -z "$engagement_tag" ] && engagement_tag="session"

    SESSION_FILE="${LOG_DIR}/${engagement_tag}_$(date +%Y%m%d-%H%M%S).cast"

    echo -e "${spaces}${greenplus} Recording to ${SESSION_FILE}"
    echo -e "${spaces}${greenplus} Type 'exit' when done to stop recording\n"

    # bind directly to the controlling terminal - banzeimykali's own stdout/stderr
    # are piped through tee for banzeimykali.log, which is not a tty asciinema can record
    asciinema rec "${SESSION_FILE}" --title "${engagement_tag}" < /dev/tty > /dev/tty 2> /dev/tty

    [ "$finduser" != "root" ] && chown $finduser:$finduser "${SESSION_FILE}"

    echo -e "\n  ${greenplus} Session recording saved: ${SESSION_FILE}"
    echo -e "${spaces}${greenplus} Replay locally:  asciinema play ${SESSION_FILE}"
    echo -e "${spaces}${greenplus} Share as text:   asciinema cat ${SESSION_FILE} | less -R"
    }
        ...
        then
          if [[ -f /usr/bin/google-chrome ]];
            then
              ...
              chrome_deb_tmp=$(mktemp --suffix=.deb)
              eval wget https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb -O ${chrome_deb_tmp}
              EXIT_STATUS="$?"
              check_exit_status ${APP} ${FUNCTYPE} ${EXIT_STATUS}
              clean_vars
...
disable_power_kde() {
    echo -e "\n  ${greenplus} KDE Plasma detected - Disabling Power Savings"

    KWRITECONFIG=""
    command -v kwriteconfig6 >/dev/null 2>&1 && KWRITECONFIG="kwriteconfig6"
    [ -z "$KWRITECONFIG" ] && command -v kwriteconfig5 >/dev/null 2>&1 && KWRITECONFIG="kwriteconfig5"

    if [ -z "$KWRITECONFIG" ]
      then
        echo -e "${spaces}${redexclaim} kwriteconfig5/kwriteconfig6 not found - skipping KDE power management changes"
        return
    fi

    for kde_power_profile in AC Battery
      do
        sudo -i -u $finduser $KWRITECONFIG --file powermanagementprofilesrc --group $kde_power_profile --group SuspendSession --key idleTime 0
        sudo -i -u $finduser $KWRITECONFIG --file powermanagementprofilesrc --group $kde_power_profile --group SuspendSession --key suspendThenHibernate false
        sudo -i -u $finduser $KWRITECONFIG --file powermanagementprofilesrc --group $kde_power_profile --group DimDisplay --key idleTime 0
        sudo -i -u $finduser $KWRITECONFIG --file powermanagementprofilesrc --group $kde_power_profile --group DPMSControl --key idleTime 0
    done
     echo -e "  ${greenplus} powermanagementprofilesrc updated - suspend/dim/dpms disabled on AC and Battery"

    sudo -i -u $finduser $KWRITECONFIG --file kscreenlockerrc --group Daemon --key Autolock false
    sudo -i -u $finduser $KWRITECONFIG --file kscreenlockerrc --group Daemon --key LockOnResume false
     echo -e "  ${greenplus} kscreenlockerrc updated - Autolock disabled\n"
    }
...
disable_power_checkde() {
    ...
    detect_kde=$(ps -e | grep -c -E '^.* plasmashell$')
...
fix_spike() {
    ...
    spike_deb_tmp=$(mktemp --suffix=.deb)
    eval wget https://old.kali.org/kali/pool/main/s/spike/spike_2.9-1kali6_$arch.deb -O ${spike_deb_tmp}
    EXIT_STATUS="$?"
    check_exit_status ${APP} ${FUNCTYPE} ${EXIT_STATUS}
    clean_vars
...
fix_gowitness() {
    ...
    if [[ $check_gowitness -eq 0 ]]
      then
        ...
        wget -q https://github.com/sensepost/gowitness/releases/download/${LATEST_VERSION}/gowitness-${LATEST_VERSION}-linux-$arch -O /usr/bin/gowitness
        EXIT_STATUS="$?"
        check_exit_status ${APP} ${FUNCTYPE} ${EXIT_STATUS}
        clean_vars
...
fix_nmap() {
    ...
    # http-shellshock.nse
    SHELLSHOCK_FIXED_NSE="${SCRIPT_DIR}/fixed-http-shellshock.nse"
...
install_sublime() {
    ...
    wget -qO - https://download.sublimetext.com/sublimehq-pub.gpg | gpg --no-default-keyring --keyring ./temp-keyring.gpg --import
    gpg --no-default-keyring --keyring ./temp-keyring.gpg --export --output sublime-text.gpg
    rm temp-keyring.gpg temp-keyring.gpg~
    mkdir -p /usr/local/share/keyrings
    mv ./sublime-text.gpg /usr/local/share/keyrings

    echo "deb [signed-by=/usr/local/share/keyrings/sublime-text.gpg] https://download.sublimetext.com/ apt/stable/" > /etc/apt/sources.list.d/sublime-text.list
    apt_update
...
fix_sead_warning() {
 ...
 "$bold$redexclaim$red" WARNING "$redexclaim$bold$red"  BANZEIMYKALI IMPACKET REMOVAL FUNCTION  "$redexclaim$bold$red" WARNING "$redexclaim$white$norm"
...
install_old_impacket() {
    install_pip2
    install_pip2_modules
    /usr/bin/pip2 uninstall impacket
    fix_impacket_array
    [ -d /opt/impacket-0.9.19 ] && rm -rf /opt/impacket-0.9.19
    impacket_tgz_tmp=$(mktemp --suffix=.tar.gz)
    wget https://github.com/SecureAuthCorp/impacket/releases/download/impacket_0_9_19/impacket-0.9.19.tar.gz -O ${impacket_tgz_tmp}
    tar xvfz ${impacket_tgz_tmp} -C /opt
    cd /opt
    chown -R root:root impacket-0.9.19
    cd /opt/impacket-0.9.19
    /usr/bin/pip2 install -r requirements.txt
    cd /opt/impacket-0.9.19
    /bin/python2.7 ./setup.py install
    rm -f ${impacket_tgz_tmp}
    ...
    echo -e "\n\n  ${greenplus} Copying everything from /home/$finduser to /root... Please wait..."
     ...
     if [[ $finduser = "root" ]]
      then
       ...
       cp -Rvf /home/$finduser/.* /home/$finduser/* /root >/dev/null 2>&1
...
pbb_lab_setup() {
    ...
    if [[ $finduser == "root" ]]
      then 
        ...
        # intentional: vulnerable-by-design training lab needs a world-writable upload dir
...
peh_weblab_setup() {
    ...
    if [[ $finduser == "root" ]]
      then 
        ...
        # intentional: vulnerable-by-design training lab needs a world-writable assets dir
...
fix_ghidra() {
    ...
    wget -q "${DOWNLOAD_URL}" -O "${GHIDRA_TMP_ZIP}"
...
#---- begin banzeimykali-mirrors (originally pimpmykali-mirrors rev 1.3.2 08.20.2021)
get_mirrorlist() {
    cleanup
    fix_sources
    echo -e "\n  ${greenplus} Banzeimykali-Mirrors - kali repo mirror speedtest"
...
# ascii art - DONT move
asciiart=$(base64 -d <<< "H4sIAAAAAAACA42P0Q0CMQxD/zOFP0E6kQUY5aTHIDc8idvCCYREIyWO7aapJIF+n2/tZAeiSipx
g9PCcrpwncK4Cm0fvoIlR0+obnd+TIznyvQQmNiRLHdFeIV85xXVjQc/yNR2P7sjudVCucOG2bll
M028KFu6HhzLTYb+Pf56RjwBVh8w53kBAAA="  | gunzip )

# 24-bit RGB rainbow anchors (closed loop: red -> orange -> yellow -> green -> cyan -> blue -> magenta -> red)
RAINBOW_R=(255 255 255   0   0   0 255 255)
RAINBOW_G=(  0 127 255 255 255   0   0   0)
RAINBOW_B=(  0   0   0   0 255 255 255   0)

print_rgb_banner() {
    local text="$1"
    local cycle_len="${2:-40}"   # terminal columns per full rainbow cycle
    local row_shift="${3:-6}"    # column shift per row, gives the diagonal sweep
    local row=0
    local esc=$'\e'

    while IFS= read -r line; do
      local out="" col=0 len=${#line}
      while [ $col -lt $len ]; do
        local ch="${line:$col:1}"
        if [ "$ch" == " " ]
          then
            ...
            local pos=$(( col + row * row_shift ))
            local phase=$(( (pos % cycle_len) * 7 * 256 / cycle_len ))
            local seg=$(( phase / 256 ))
            local frac=$(( phase % 256 ))
            local r=$(( RAINBOW_R[seg] + (RAINBOW_R[seg+1] - RAINBOW_R[seg]) * frac / 256 ))
            local g=$(( RAINBOW_G[seg] + (RAINBOW_G[seg+1] - RAINBOW_G[seg]) * frac / 256 ))
            local b=$(( RAINBOW_B[seg] + (RAINBOW_B[seg+1] - RAINBOW_B[seg]) * frac / 256 ))
            # use a real ESC byte (not the literal text "\e") since the slant font
            # itself contains literal backslashes - echo -e would otherwise merge
            # a glyph's trailing "\" with the next escape code's "\e" into "\\e"
...
banzeimykali_menu() {
    while true; do
      ...
      echo -e "  0 - Fix All                       (runs fix_missing plus autoremove/fix-broken/vm checks) \n"   # fix_all
      echo -e "  "$bold"N - NEW VM SETUP"$reset" - Run this option if this is the first time running banzeimykali\n"
      echo -e "  = - Banzeimykali-Mirrors          (find fastest kali mirror. use the equals symbol = )"          # get_mirrorlist; best_ping; small_speedtest; large_speedtest; gen_new_sources; cleanup;;
      ...
      echo -e "  % - CrackMapExec                  (install crackmapexec)"                                       # fix_cme
      echo -e "  C - Certipy                       (install certipy-ad, AD CS enumeration/abuse)"                 # fix_certipy
      echo -e "  D - Coercer                       (install coercer)"                                            # fix_coercer
      echo -e "  F - AutoRecon                     (install autorecon + recon dependencies)"                     # fix_autorecon
      echo -e "  L - Start Engagement Log          (asciinema terminal session recorder)"                        # start_engagement_log
      echo -e "  R - ProjectDiscovery Suite        (subfinder httpx-toolkit nuclei dnsx katana)"                  # fix_projectdiscovery
      echo -e "  W - Clean Kali                    (apt clean/autoremove, journalctl vacuum, optional VM Zero-Out)"  # clean_kali
      echo -e "  ^ - Install Everything            (apt install kali-linux-everything, enables root login, multi-GB)\n" # install_everything
      echo -e "  Please use sudo ./banzeimykali.sh --help for additional installations/fixes\n"
      # menu selection
...
#      j|J) ;;
      ...
      l|L) start_engagement_log;;
#      m|M) ;;
      n|N) fix_all;;
      o|O) hacking_api_prereq;;
      p|P) fix_linwinpeas;;
#      q|Q) ;;
      r|R) fix_projectdiscovery;;
      s|S) ;; # new_kali_signingkey executed
      t|T) fix_timezone;;
      u|U) fix_netexec;;
      v|V) install_vscode;;
      w|W) clean_kali;;
      x|X) echo -e "\n\n Exiting banzeimykali.sh - Happy Hacking! \n";;
...
banzeimykali_help() {
    ...
    "    --autorecon  install autorecon and its recon tool dependencies"
    ...
    "      --certipy  install certipy-ad"
    "    --cleankali  apt clean, autoremove --purge, journalctl vacuum, optional VM Zero-Out"
    "          --cme  install crackmapexec"
    "      --checkvm  detect hypervisor, install guest additions"
    "      --coercer  install coercer"
    "--dockercompose  install docker compose"
    "         --dnsx  install dnsx"
    "   --everything  apt install kali-linux-everything, enables root login, multi-GB"
    ...
    "        --httpx  install httpx-toolkit"
    "     --impacket  install impacket latest"
    " --nukeimpacket  install impacket 0.9.19, leave python3 as default"
    "       --katana  install katana (go install, projectdiscovery)"
    "      --mirrors  run banzeimykali-mirrors speedtest"
    ...
    "      --pdsuite  install projectdiscovery suite (subfinder httpx-toolkit nuclei dnsx katana)"
    "    --plumhound  install plumhound"
    "         --root  set speedrun var, enable root login"
    "      --smbconf  run fix smb.conf "
    "     --seclists  install seclists"
    "        --spike  run fix spike"
    "     --startlog  start an asciinema engagement session recording"
    ...
    "      --zeroout  zero-out free disk space for VM Compact/Compress (Ctrl+C safe to abort)"
    "\n Python fixes:"
...
check_arg() {
  ...
  --binfmt-winexe) setup_binfmt_misc_winexe;;
     ...
     --brokenxfce) fix_broken_xfce;;
        --certipy) fix_certipy;;
      --cleankali) clean_kali;;
            --cme) fix_cme;;
        --checkvm) virt_what; check_vm;;
        --coercer) fix_coercer;;
  --dockercompose) fix_dockercompose;;
           --dnsx) fix_dnsx;;
    --everything) SPEEDRUN=1; ENABLE_ROOT=1; install_everything;;
   ...
   --nukeimpacket) SPEEDRUN=1; install_pip2; install_pip3; fix_pip2_pip3; fix_sead_warning;;
      ...
      --pehweblab) fix_libwacom; peh_weblab_setup;;
        ...
        --zeroout) clean_kali_zeroout;;
                *) banzeimykali_help; exit 0;;
        esac
      fi
    }
